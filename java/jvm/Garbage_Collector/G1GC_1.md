...
- **무엇을 수집하나?**  
  모든 Eden Region과 Survivor Region을 대상으로 살아 있는 객체를 찾아, 비어 있는 Region으로 **복사(Copying)** 한다.  
  이전 Region은 완전히 비운다.
...
4. **Cleanup**  
   어떤 Old Region이 비어 있거나, 얼마나 많은 쓰레기를 포함하는지 계산한다.  
   이 결과는 다음 Mixed GC에서 수집할 Region을 고르는 데 사용된다.  
   간단한 STW를 포함할 수 있다.

- **SATB(Snapshot-At-The-Beginning) 방식 사용**  
  G1GC는 마킹의 정확성을 위해 **GC 시작 시점의 객체 참조 상태를 기준으로 생존 여부를 판단**한다.  
  이를 통해 병행 중 객체 참조가 바뀌더라도 정확한 마킹이 가능하다.

> **결과**: 이 과정을 통해 G1은  
> "어떤 Old Region이 Garbage로 가득 차 있는지"에 대한 정확한 정보를 확보하게 된다.
